 ...
 */
public class CustomDefaultClusterRenderer<T extends CustomClusterItem> implements CustomClusterRenderer<T> {
    private final GoogleMap mMap;
    private final IconGenerator mIconGenerator;
    private final CustomClusterManager<T> mClusterManager;
    ...
    private CustomClusterManager.OnClusterClickListener<T> mClickListener;
    private CustomClusterManager.OnClusterInfoWindowClickListener<T> mInfoWindowClickListener;
    private CustomClusterManager.OnClusterInfoWindowLongClickListener<T> mInfoWindowLongClickListener;
    private CustomClusterManager.OnClusterItemClickListener<T> mItemClickListener;
    private CustomClusterManager.OnClusterItemInfoWindowClickListener<T> mItemInfoWindowClickListener;
    private CustomClusterManager.OnClusterItemInfoWindowLongClickListener<T> mItemInfoWindowLongClickListener;

    public CustomDefaultClusterRenderer(Context context, GoogleMap map, CustomClusterManager<T> clusterManager) {
        mMap = map;
        mAnimate = true;
    ...
    public void onAdd() {
        ...
        mClusterManager.getClusterMarkerCollection().setOnMarkerClickListener(
                marker -> mClickListener != null && mClickListener.onClusterClick(mClusterMarkerCache.get(marker), marker));
    ...
    private class RenderTask implements Runnable {
        ...
        @SuppressLint("NewApi")
        public void run() {
            if (!shouldRender(immutableOf(CustomDefaultClusterRenderer.this.mClusters), immutableOf(clusters))) {
                mCallback.run();
                return;
            }
            ...
            if (CustomDefaultClusterRenderer.this.mClusters != null && mAnimate) {
    ...
    @Override
    public void setOnClusterClickListener(CustomClusterManager.OnClusterClickListener<T> listener) {
        mClickListener = listener;
    }

    @Override
    public void setOnClusterInfoWindowClickListener(CustomClusterManager.OnClusterInfoWindowClickListener<T> listener) {
        mInfoWindowClickListener = listener;
    }

    @Override
    public void setOnClusterInfoWindowLongClickListener(CustomClusterManager.OnClusterInfoWindowLongClickListener<T> listener) {
        mInfoWindowLongClickListener = listener;
    }

    @Override
    public void setOnClusterItemClickListener(CustomClusterManager.OnClusterItemClickListener<T> listener) {
        mItemClickListener = listener;
    }

    @Override
    public void setOnClusterItemInfoWindowClickListener(CustomClusterManager.OnClusterItemInfoWindowClickListener<T> listener) {
        mItemInfoWindowClickListener = listener;
    }

    @Override
    public void setOnClusterItemInfoWindowLongClickListener(CustomClusterManager.OnClusterItemInfoWindowLongClickListener<T> listener) {
     ...
     * <p>
     * The first time {@link CustomClusterManager#cluster()} is invoked on a set of items
     ...
     * If an item is removed and re-added (or updated) and {@link CustomClusterManager#cluster()} is
     ...
     * re-added (or updated) and {@link CustomClusterManager#cluster()} is invoked
